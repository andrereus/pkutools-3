...
export async function checkPremiumStatus(userId: string): Promise<boolean> {
  ...
  return licenseKey === config.pkutoolsLicenseKey || licenseKey === config.pkutoolsLicenseKey2
}

/**
 * Checks if a user has premium+AI license status
 * @param userId - The user's Firebase UID
 * @returns Promise<boolean> - True if user has premium+AI license
 */
export async function checkPremiumAIStatus(userId: string): Promise<boolean> {
  ...
  const config = useRuntimeConfig()
  const licenseKey = settings.license
  return licenseKey === config.pkutoolsLicenseKey2
}
